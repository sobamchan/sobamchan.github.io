...
a lot of string open-weight text encoders are now trained with, as well as, proprietary apis.
however, there is almost now study (afaik) which analysis models trained with mrl, and it bothers me a bit.
...
i used bert-based models i trained, computed embeddings of some documents (texts from [this dataset](https://huggingface.co/datasets/zeta-alpha-ai/NanoMSMARCO)), and computed three metrics, isoscore (↑), uniform loss (↓), correlation between dimensions (↓).
